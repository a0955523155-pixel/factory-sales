...
const Home = () => {
  ...
  // 過濾邏輯
  const filteredProps = searchRegion 
    ? properties.filter(p => p.basicInfo.address.includes(searchRegion))
    : properties;
    ...
    <div className="min-h-screen tech-bg-light font-sans selection:bg-orange-500 selection:text-white pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="relative z-10">
          ...
          <div className="max-w-md relative mt-8">
             <select 
               ...
               className="w-full p-4 pl-12 rounded-full border-2 border-slate-200 focus:border-orange-500 focus:outline-none shadow-lg text-lg appearance-none bg-white cursor-pointer"
             >
               <option value="">顯示所有地區物件</option>
      ...
      <div className="pl-6 max-w-7xl mx-auto w-full relative z-10 overflow-hidden mb-20">
        <div className="flex items-center justify-between mb-6 pr-6 border-b border-slate-200 pb-4">
          <h2 className="text-3xl font-bold flex items-center gap-3 text-slate-800"><span className="w-3 h-8 bg-orange-600 block"></span>精選案場</h2>
          <div className="flex gap-4 items-center">
             <span className="font-mono text-slate-400 text-sm font-bold hidden md:block">SCROLL &rarr;</span>
             <span className="font-mono text-slate-400 text-lg font-bold">DATA: {filteredProps.length}</span>
          </div>
        </div>
      ...
      <div className="bg-slate-100 py-20 border-t border-slate-200">
         <div className="max-w-7xl mx-auto px-6">
            <div className="flex justify-between items-end mb-10">
               ...
               <Link to="/news" className="flex items-center gap-1 font-bold text-orange-600 hover:underline">查看更多 <ArrowUpRight size={18}/></Link>
            </div>
