...
import { ArrowRight, MapPin, Newspaper, Megaphone, Flame, FileText } from 'lucide-react';

// ★★★ 功能：最強純文字淨化器 (處理 HTML 標籤與 &nbsp; 等亂碼) ★★★
  ...
  return (
    <div className="font-sans text-slate-900 bg-white selection:bg-orange-100 selection:text-orange-900">
      ...
      <Helmet>
        ...
        <meta name="description" content="專注於高雄與屏東地區的工業地產開發與銷售。綠芽團隊提供專業的廠房買賣、工業地出租、農地與建地投資諮詢服務。" />
        <meta name="keywords" content="高雄廠房, 屏東廠房, 工業地買賣, 廠房出租, 綠芽團隊, 高雄不動產, 大成工業城, 九大工業城" />
      ...
      <header className="relative h-screen flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img 
            // ★★★ 修正：優先使用後台設定的 heroBgUrl，若無則顯示預設圖 ★★★
            src={globalSettings.heroBgUrl || "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=2070&auto=format&fit=crop"} 
            ...
            {latestNews.map((news) => (
              <Link to={`/article/${news.id}`} key={news.id} className="group block cursor-pointer">
                <div className="aspect-video w-full mb-4 overflow-hidden rounded-2xl bg-slate-100 shadow-sm">
                  {news.image ? (
                    <img src={news.image} alt="" className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                  ) : (
                ...
                {/* ★★★ 標題：使用淨化器完美處理亂碼 ★★★ */}
                <h3 className="text-lg font-bold text-slate-900 mb-2 group-hover:text-orange-600 transition line-clamp-2">
                  {stripHtmlAndEntities(news.title || '無標題')}
                </h3>
                
                {/* ★★★ 內文摘要：過濾標籤並擷取前 60 字 ★★★ */}
                <p className="text-slate-500 text-sm mb-4 line-clamp-2 leading-relaxed">
                  {stripHtmlAndEntities(news.content).substring(0, 60)}...
                </p>
