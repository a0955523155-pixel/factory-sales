...
const Home = () => {
  ...
  const getCatName = (cat) => {
    switch(cat) {
      case 'news': return '市場消息';
      case 'academy': return '房地產小學堂';
      case 'cases': return '成交案例';
      default: return '動態';
    }
  };
  ...
  const getCatColor = (cat) => {
    switch(cat) {
      ...
      case 'cases': return 'bg-purple-500';
    ...
    <div className="min-h-screen tech-bg-light font-sans selection:bg-orange-500 selection:text-white pb-20">
      ...
      <div className="pl-6 max-w-7xl mx-auto w-full relative z-10 overflow-hidden mb-20">
        ...
        {loading ? <div className="text-center py-20 text-2xl font-mono text-slate-400">LOADING DATA...</div> : (
          <div className="flex gap-8 overflow-x-auto pb-12 snap-x snap-mandatory pr-6 scrollbar-hide" style={{ scrollBehavior: 'smooth' }}>{filteredProps.map((item, index) => (<motion.div key={item.id} initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="snap-center shrink-0 w-[85vw] md:w-[400px]"><Link to={`/property/${item.id}`} className="group block bg-white border border-slate-200 hover:border-orange-500 transition-all duration-300 relative overflow-hidden rounded-2xl shadow-sm hover:shadow-2xl hover:-translate-y-2 h-full flex flex-col"><div className="h-64 overflow-hidden relative"><img src={item.basicInfo?.thumb || "https://via.placeholder.com/400x300"} alt={item.basicInfo?.title} className="w-full h-full object-cover transition duration-700 group-hover:scale-110"/><div className="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1 text-xs font-bold font-mono border border-slate-200 rounded">ID: {item.id.slice(0,6)}</div></div><div className="p-8 flex-1 flex flex-col"><div className="mb-4"><h3 className="text-2xl font-bold text-slate-900 group-hover:text-orange-600 transition line-clamp-1">{item.basicInfo?.title}</h3><p className="text-slate-500 mt-1 line-clamp-1">{item.basicInfo?.subtitle}</p></div><div className="flex items-center gap-2 text-slate-500 mb-6 pb-6 border-b border-slate-100"><MapPin size={18} className="text-orange-500" /><span className="text-lg line-clamp-1">{item.basicInfo?.address}</span></div><div className="flex justify-between items-center mt-auto"><span className="text-2xl font-black text-slate-900">{item.basicInfo?.price}</span><div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center group-hover:bg-orange-600 group-hover:text-white transition"><ArrowRight size={20}/></div></div></div></Link></motion.div>))}
            <div className="w-4 shrink-0"></div></div>
        )}
      </div>
                  ...
                  articles.map((article) => (
                     <Link to={`/${article.category}`} key={article.id} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition duration-300 group flex flex-col h-full relative">
                        <div className="h-48 overflow-hidden relative">
                           ...
                           <span className={`absolute top-4 left-4 px-3 py-1 text-xs text-white font-bold rounded shadow ${getCatColor(article.category)}`}>
