          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-8 max-w-4xl mx-auto w-full">
                <div className="space-y-12 pb-20">
                  ...
                  <section className="space-y-4"><div className="flex justify-between"><h3 className="font-bold text-lg border-l-4 border-orange-500 pl-2">周遭環境 / 新聞連結</h3><div className="flex gap-2"><button type="button" onClick={openSearch} className="text-blue-600 text-xs flex items-center gap-1 border border-blue-200 px-2 py-1 rounded hover:bg-blue-50"><Search size={14}/> 搜尋工商時報</button><button type="button" onClick={()=>setEnvList([...envList, {id: Date.now(), title:"", desc:"", image:"", link:""}])} className="text-orange-500 text-xs flex items-center gap-1"><Plus size={14}/> 新增</button></div></div>{envList.map((env, i) => (<div key={i} className="bg-white p-2 border rounded mb-2"><input value={env.title} onChange={e=>{const x=[...envList];x[i].title=e.target.value;setEnvList(x)}} className={inputStyle} placeholder="標題"/><textarea value={env.desc} onChange={e=>{const x=[...envList];x[i].desc=e.target.value;setEnvList(x)}} className={inputStyle} placeholder="描述"/><input type="file" onChange={e=>handleUpload(e, (url)=>{const x=[...envList];x[i].image=url;setEnvList(x)})} className="text-xs"/><input value={env.link} onChange={e=>{const x=[...envList];x[i].link=e.target.value;setEnvList(x)}} className={inputStyle} placeholder="連結"/></div>))}</section>
