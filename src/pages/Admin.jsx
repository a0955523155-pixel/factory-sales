...
import { 
  ...
  Star, Award, History, Search, Train, Factory, MapPin 
} from 'lucide-react';
...
const Admin = () => {
  ...
  const handleSmartNewsSearch = (type) => {
    ...
    switch (type) {
      case 'traffic':
        keywords = `${city} ${district} 交通建設 捷運 國道 高鐵 延伸`;
        break;
      case 'industry':
        keywords = `${city} ${district} 產業園區 科學園區 招商 設廠`;
        break;
      case 'area':
      default:
        keywords = `${city} ${district} 重劃區 建設 利多 房市`;
        break;
    }
  ...
  const handleSmartNewsGenerate = () => {
    ...
    if (type === '工業地') {
      title = `${district}產業聚落成形，增值潛力看漲`;
      desc = `受惠於南部半導體S廊帶效應，${district}近期產業用地需求大增。本案鄰近主要交通幹道，物流運輸便捷，且周邊供應鏈完整，適合企業設廠或倉儲物流佈局，掌握產業起飛的最佳時機。`;
    } else if (type === '農地') {
      title = `${district}交通建設到位，土地價值重估`;
      desc = `隨著國土計畫法即將上路，${district}優質農地日益稀缺。本案地形方正，臨路條件佳，具備長期置產抗通膨效益。加上周邊交通路網持續優化，未來發展潛力不容小覷。`;
    } else if (type === '建地') {
      title = `${district}生活機能躍升，宜居熱點首選`;
      desc = `近年${district}人口持續移入，生活機能日趨成熟。本案位處核心地段，鄰近學區與商圈，交通便利性高。隨著區域內重大建設陸續完工，將成為自建豪宅或開發推案的絕佳基地。`;
    } else {
      title = `${district}重大建設齊發，區域發展加速`;
      desc = `政府積極推動${district}基礎建設，帶動區域轉型升級。本案坐擁地利之便，享有交通與產業雙重紅利，不論是投資置產或自用開發，都是極具競爭力的優質選擇。`;
    }
    
    const newEnv = [...envList];
    if (newEnv.length > 0 && newEnv[newEnv.length - 1].title === "") {
        newEnv[newEnv.length - 1] = { ...newEnv[newEnv.length - 1], title, desc };
    } else {
        newEnv.push({ id: Date.now(), title, desc, image: "", link: "" });
    }
    setEnvList(newEnv);
  };
