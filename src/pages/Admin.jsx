...
import { X, Plus, Trash2, Layout, Users, Settings, Map as MapIcon, Upload, Languages, FileText, Sparkles, LogIn, LogOut, GripVertical, ChevronUp, ChevronDown, RefreshCcw, Copy } from 'lucide-react';
...
const Admin = () => {
  ...
  const [units, setUnits] = useState([{ 
    id: 'u1', number: '', ping: '', unitPrice: '', price: '', status: 'available', layout: '' 
  }]);
  ...
  const handleUnitChange = (index, field, value) => {
    ...
    if (field === 'ping' || field === 'unitPrice') {
      const ping = newUnits[index].ping;
      const unitPrice = newUnits[index].unitPrice;
      newUnits[index].price = calculateTotalPrice(ping, unitPrice);
    }
    setUnits(newUnits);
  };
  ...
  const resetForm = () => {
    setEditId(null);
    setFormData({ title: '', titleEN: '', subtitle: '', description: '', price: '', address: '', city: '高雄', propertyType: '工業地', usageType: '廠房', agentPhone: '', agentName: '', lineId: '', lineQr: '', googleMapUrl: '', thumb: '', images: [] });
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 max-w-5xl mx-auto w-full">
                <div className="space-y-10 pb-20">
                  ...
                  {/* Unit List (Simplified, Auto Calculation, New at Top) */}
                  <section className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
                    <div className="flex justify-between items-center mb-6"><h3 className="font-black text-lg border-l-4 border-orange-500 pl-3">戶別銷控表 (Unit List)</h3><button onClick={handleAddUnit} className="bg-orange-50 text-orange-600 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-orange-100 transition flex items-center gap-1"><Plus size={14}/> 新增戶別</button></div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                       {units.map((u, i) => (
                          <div key={i} className="p-4 border border-slate-200 rounded-xl bg-slate-50 hover:bg-white hover:border-orange-300 transition shadow-sm relative group">
                             <div className="grid grid-cols-2 gap-3 mb-3">
                                <div className="col-span-1"><label className="text-[10px] text-slate-400 font-bold block uppercase mb-1">戶號</label><input value={u.number} onChange={e=>handleUnitChange(i, 'number', e.target.value)} className="w-full bg-white border rounded p-1.5 text-sm font-bold text-center" placeholder="A1"/></div>
                                <div className="col-span-1"><label className="text-[10px] text-slate-400 font-bold block uppercase mb-1">狀態</label><select value={u.status} onChange={e=>handleUnitChange(i, 'status', e.target.value)} className="w-full bg-white border rounded p-1.5 text-xs"><option value="available">🟢 可銷售</option><option value="reserved">🟡 已預訂</option><option value="sold">🔴 已售出</option></select></div>
                                <div className="col-span-1"><label className="text-[10px] text-slate-400 font-bold block uppercase mb-1">坪數</label><input value={u.ping} onChange={e=>handleUnitChange(i, 'ping', e.target.value)} className="w-full border-b bg-transparent p-1 text-sm focus:outline-none focus:border-orange-500" placeholder="0"/></div>
                                <div className="col-span-1"><label className="text-[10px] text-slate-400 font-bold block uppercase mb-1">單價 (萬/坪)</label><input value={u.unitPrice} onChange={e=>handleUnitChange(i, 'unitPrice', e.target.value)} className="w-full border-b bg-transparent p-1 text-sm focus:outline-none focus:border-orange-500 text-blue-600" placeholder="0.0"/></div>
                             ...
                             <div className="flex justify-between items-center border-t border-slate-200 pt-2">
                                <label className="cursor-pointer text-xs text-blue-500 hover:text-blue-700 font-bold flex items-center gap-1"><Upload size={12}/> {u.layout ? "更換圖檔" : "上傳平面圖"}<input type="file" className="hidden" onChange={e=>handleUpload(e, (url)=>{const x=[...units];x[i].layout=url;setUnits(x)})}/></label>
                                <div className="flex gap-2">
                                   <button onClick={() => handleDuplicateUnit(u)} className="text-slate-400 hover:text-green-600" title="複製戶別"><Copy size={14}/></button>
                                   <button onClick={()=>setUnits(units.filter((_,idx)=>idx!==i))} className="text-slate-300 hover:text-red-500"><Trash2 size={14}/></button>
                                </div>
                             </div>
                          </div>
                       ))}
