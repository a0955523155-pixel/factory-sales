      ...
      <div className="h-screen flex items-center justify-center bg-slate-100 px-4">
        <form onSubmit={handleLogin} className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm border border-slate-200">
          ...
          <div className="space-y-4">
             <input type="text" placeholder="帳號" value={loginForm.user} onChange={e=>setLoginForm({...loginForm, user:e.target.value})} className="w-full p-3 border rounded-xl bg-slate-50 focus:border-orange-500 outline-none" autoComplete="username" />
             <input type="password" placeholder="密碼" value={loginForm.pass} onChange={e=>setLoginForm({...loginForm, pass:e.target.value})} className="w-full p-3 border rounded-xl bg-slate-50 focus:border-orange-500 outline-none" autoComplete="current-password" />
             ...
             <div className="flex-1 p-6 md:p-10 overflow-y-auto bg-slate-50">
                <div className="max-w-3xl mx-auto">
                   ...
                   <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><label className={labelStyle}>文章分類</label><select value={articleForm.category} onChange={e=>setArticleForm({...articleForm, category: e.target.value})} className={inputStyle}><option value="news">市場最新消息</option><option value="academy">房地產小學堂</option><option value="cases">成交案例分享</option></select></div><div><label className={labelStyle}>發布日期</label><input type="date" value={articleForm.date} onChange={e=>setArticleForm({...articleForm, date: e.target.value})} className={inputStyle}/></div></div>
                      <div><label className={labelStyle}>文章標題</label><input value={articleForm.title} onChange={e=>setArticleForm({...articleForm, title: e.target.value})} className={inputStyle} placeholder="請輸入吸引人的標題..."/></div>
                      <div className="relative"> <div className="flex justify-between items-center mb-1"> <label className={labelStyle}>文章內容</label> <button type="button" onClick={handleAIWrite} className="text-xs flex items-center gap-1 text-purple-600 font-bold hover:text-purple-800 bg-purple-50 px-2 py-1 rounded transition"><Sparkles size={12}/> AI 自動撰寫</button> </div> <textarea value={articleForm.content} onChange={e=>setArticleForm({...articleForm, content: e.target.value})} className={`${inputStyle} h-64 leading-relaxed`} placeholder="輸入內容，或點擊 AI 自動撰寫..."/></div>
                      <div><label className={labelStyle}>封面圖片 (自動壓浮水印)</label><div className="flex items-center gap-4"><label className="cursor-pointer bg-slate-100 hover:bg-slate-200 px-4 py-3 rounded-lg flex items-center gap-2 text-sm font-bold text-slate-600 transition"><Upload size={16}/> 上傳圖片 <input type="file" className="hidden" onChange={e=>handleUpload(e, (url)=>setArticleForm({...articleForm, image: url}))}/></label>{articleForm.image && <img src={articleForm.image} className="h-20 w-32 object-cover rounded-lg border border-slate-200 shadow-sm"/>}</div></div>
                   </div>
                </div>
             </div>
          </div>
