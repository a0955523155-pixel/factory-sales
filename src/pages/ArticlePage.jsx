...
import { Calendar, ArrowRight, MapPin, Filter, Search, Building2, Ruler, Banknote, Map, Briefcase, Award, Globe } from 'lucide-react';
...
const ArticlePage = ({ categoryGroup, category, title }) => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0); // 確保進入列表時在最上面
    const fetchData = async () => {
      setLoading(true);
      try {
        const collectionName = isWorksPage ? "properties" : "articles";
        const q = query(collection(db, collectionName), orderBy(isWorksPage ? "updatedAt" : "createdAt", "desc"));
        ...
        snap.forEach((doc) => {
          ...
          if (!isWorksPage) {
             ...
             const districtMatch = item.basicInfo?.address?.match(/(?:縣|市)(\S+?(?:區|鄉|鎮|市))/);
  ...
  const filteredWorks = useMemo(() => {
    ...
    return dataList.filter(item => {
      ...
      if (filters.price !== 'All') {
        const p = parseFloat(info.price?.replace(/[^0-9.]/g, '') || 0);
        if (filters.price === '<1000' && p >= 1000) return false;
        if (filters.price === '1000-3000' && (p < 1000 || p > 3000)) return false;
        if (filters.price === '>6000' && p <= 6000) return false;
      }
      if (filters.ping !== 'All') {
  ...
  const groupedWorks = useMemo(() => {
    const groups = {};
    filteredWorks.forEach(item => { const type = item.basicInfo.propertyType || '其他'; if (!groups[type]) groups[type] = []; groups[type].push(item); });
    Object.keys(groups).forEach(k => groups[k].sort((a, b) => (b.basicInfo.isFeaturedWork === true ? 1 : 0) - (a.basicInfo.isFeaturedWork === true ? 1 : 0)));
    return groups;
  }, [filteredWorks]);

  const availableDistricts = useMemo(() => { const districts = new Set(dataList.map(i => i.computedDistrict).filter(Boolean)); return ['All', ...Array.from(districts).sort()]; }, [dataList]);
  const FilterSelect = ({ icon: Icon, label, value, onChange, options }) => ( <div className="flex flex-col gap-1 w-full sm:w-auto flex-1 min-w-[140px]"><label className="text-xs font-bold text-slate-400 flex items-center gap-1"><Icon size={10}/> {label}</label><select value={value} onChange={(e) => onChange(e.target.value)} className="w-full bg-white border border-slate-200 text-slate-700 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-orange-500 font-bold shadow-sm">{options.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}</select></div> );

  return (
    <div className="font-sans min-h-screen bg-slate-50 flex flex-col">
      <Navbar />
      <div className="bg-slate-900 pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto text-center"><h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight">{title}</h1><div className="w-20 h-1 bg-orange-500 mx-auto rounded-full"></div></div>
      </div>
      ...
      {isWorksPage ? (
        ...
        <div className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full">
          ...
          {loading ? <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-12 w-12 border-t-4 border-orange-500"></div></div> : 
              ...
              {Object.entries(groupedWorks).map(([type, items]) => (
                <div key={type} className="animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {items.map((item) => (
                      <Link to={`/property/${item.id}`} key={item.id} className={`group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition duration-500 border flex flex-col h-full transform hover:-translate-y-1 ${item.basicInfo.isFeaturedWork ? 'border-orange-200 ring-2 ring-orange-100' : 'border-slate-100'}`}>
                        <div className="relative h-64 overflow-hidden">
                          ...
                          <div className="absolute top-4 left-4 flex gap-2">
                             ...
                             <span className="bg-slate-900/80 text-white text-[10px] font-bold px-2 py-1 rounded backdrop-blur-sm">{item.basicInfo.city}</span>
                          </div>
                          <div className="absolute bottom-4 left-4 text-white"><h3 className="text-xl font-bold leading-tight mb-1 drop-shadow-md">{item.basicInfo.title}</h3><p className="text-xs opacity-90 font-mono">{item.computedPing > 0 ? `總坪數約 ${item.computedPing} 坪` : '詳情請洽專員'}</p></div>
                        </div>
                        <div className="p-5 flex flex-col flex-1"><div className="flex justify-between items-center mb-4"><span className="text-2xl font-black text-orange-600">{item.basicInfo.price}</span><span className="text-xs text-slate-400 font-bold border border-slate-200 px-2 py-1 rounded">{item.basicInfo.usageType}</span></div><p className="text-slate-500 text-sm line-clamp-2 mb-4 flex-1">{item.basicInfo.description}</p><div className="pt-4 border-t border-slate-100 flex items-center justify-between text-sm font-bold text-slate-600 group-hover:text-orange-600 transition"><span>了解更多</span><ArrowRight size={16}/></div></div>
                      </Link>
                    ))}
                  </div>
                </div>
        ...
        <div className="flex-1 max-w-7xl mx-auto px-6 py-16 w-full">
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-400 bg-white rounded-3xl border border-dashed border-slate-300"><p className="text-lg font-bold">目前尚無相關文章</p></div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {dataList.map((article) => (
                <Link to={`/article/${article.id}`} key={article.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition duration-300 group border border-slate-100 flex flex-col h-full">
