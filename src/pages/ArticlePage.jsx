...
import { Calendar, ArrowRight, MapPin, Filter, Search, Building2, Ruler, Banknote, Map, Briefcase, Award } from 'lucide-react';
...
const ArticlePage = ({ categoryGroup, category, title }) => {
  ...
  // 篩選狀態：加入 transaction (交易類別) 與 usage (用途)
  const [filters, setFilters] = useState({
    city: 'All',
    district: 'All',
    type: 'All', // 屬性 (工業地/建地...)
    transaction: 'All', // 交易 (出售/出租)
    usage: 'All', // 用途 (廠房/農地...)
    price: 'All',
    ping: 'All'
  });
        ...
        snap.forEach((doc) => {
          ...
          if (!isWorksPage) {
             ...
             list.push({ 
                id: doc.id, 
                ...item, 
                computedPing: totalPing,
                computedDistrict: district
             });
          }
        });
  ...
  const groupedWorks = useMemo(() => {
    ...
    filteredWorks.forEach(item => {
      const type = item.basicInfo.propertyType || '其他';
  ...
  const FilterSelect = ({ icon: Icon, label, value, onChange, options }) => (
    <div className="flex flex-col gap-1 w-full sm:w-auto flex-1 min-w-[140px]">
      ...
      <select 
        ...
        className="w-full bg-white border border-slate-200 text-slate-700 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-orange-500 font-bold shadow-sm"
      >
        {options.map(opt => (
      ...
      {isWorksPage ? (
        <div className="flex-1 max-w-7xl mx-auto px-6 py-12 w-full">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 mb-12 -mt-24 relative z-10">
             <div className="flex items-center gap-2 mb-4 border-b border-slate-100 pb-3">
               ...
               <span className="font-black text-slate-800 text-lg">快速找房</span>
             </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-400 bg-white rounded-3xl border border-dashed border-slate-300">
               ...
               <p className="text-lg font-bold">沒有符合篩選條件的物件</p>
            </div>
          ) : (
              ...
              {Object.entries(groupedWorks).map(([type, items]) => (
                <div key={type} className="animate-fadeIn">
                  <div className="flex items-center gap-4 mb-8">
                     <h2 className="text-3xl font-black text-slate-800">{type}精選</h2>
                     <div className="h-[1px] flex-1 bg-slate-200"></div>
                     <span className="text-sm font-bold text-slate-400 bg-white px-3 py-1 rounded-full border border-slate-200">{items.length} 個案件</span>
                  </div>
                      ...
                      <Link to={`/property/${item.id}`} key={item.id} className={`group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition duration-500 border flex flex-col h-full transform hover:-translate-y-1 ${item.basicInfo.isFeaturedWork ? 'border-orange-200 ring-2 ring-orange-100' : 'border-slate-100'}`}>
                        <div className="relative h-64 overflow-hidden">
                          ...
                          <div className="absolute bottom-4 left-4 text-white">
                             <h3 className="text-xl font-bold leading-tight mb-1 drop-shadow-md">{item.basicInfo.title}</h3>
                             <p className="text-xs opacity-90 font-mono">{item.computedPing > 0 ? `總坪數約 ${item.computedPing} 坪` : '詳情請洽專員'}</p>
                          </div>
                        </div>
                        <div className="p-5 flex flex-col flex-1">
                          <div className="flex justify-between items-center mb-4">
                             <span className="text-2xl font-black text-orange-600">{item.basicInfo.price}</span>
                             <span className="text-xs text-slate-400 font-bold border border-slate-200 px-2 py-1 rounded">{item.basicInfo.usageType}</span>
                          </div>
                          <p className="text-slate-500 text-sm line-clamp-2 mb-4 flex-1">{item.basicInfo.description}</p>
                          <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-sm font-bold text-slate-600 group-hover:text-orange-600 transition">
                             <span>了解更多</span>
                             <ArrowRight size={16}/>
                          </div>
                        </div>
        ...
        <div className="flex-1 max-w-7xl mx-auto px-6 py-16 w-full">
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-400 bg-white rounded-3xl border border-dashed border-slate-300">
               <p className="text-lg font-bold">目前尚無相關文章</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div key={article.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition duration-300 group border border-slate-100 flex flex-col h-full">
                  <div className="aspect-video w-full overflow-hidden bg-slate-100 relative">
                     {article.image ? (
                       ...
                       <div className="w-full h-full flex items-center justify-center text-slate-300 bg-slate-50 font-black text-2xl">NO IMAGE</div>
                     )}
                     <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold text-slate-800 shadow-sm flex items-center gap-1">
                        <Calendar size={12} className="text-orange-500"/> {article.date}
                     </div>
                  </div>
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="text-xl font-bold text-slate-800 mb-3 line-clamp-2 leading-tight group-hover:text-orange-600 transition">
                      {article.title}
                    </h3>
                    ...
                    <button className="text-orange-600 font-bold text-sm flex items-center gap-1 group-hover:gap-2 transition-all mt-auto">
