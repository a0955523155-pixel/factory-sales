...
// FIX: 加入 Sparkles 和 Copy
import { 
  X, Plus, Trash2, Map as MapIcon, Upload, Languages, History, Search, Train, Factory, MapPin, Star, Award, Zap, Folder, FolderOpen, ChevronDown, Sparkles, Copy
} from 'lucide-react';
...
const PropertyPanel = () => {
  ...
  const loadEdit = (item) => { 
    ...
    setSpecs(Array.isArray(item.specs) ? item.specs : []); setFeatures(Array.isArray(item.features) ? item.features : []); setEnvList(Array.isArray(item.environmentList) ? item.environmentList : []); setProgressList(Array.isArray(item.progressHistory) ? item.progressHistory : []); setUnits(Array.isArray(item.units) ? item.units : []); 
  };
  ...
  const handleDeleteProperty = async (e, id) => { 
      e.stopPropagation(); if (!window.confirm("刪除？")) return; 
      await deleteDoc(doc(db, "properties", id)); fetchProperties(); 
  };
