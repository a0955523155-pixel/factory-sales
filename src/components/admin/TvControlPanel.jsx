...
function DraggableObject({ obj, isSelected, onSelect, onUpdate, transformMode, setIsGlobalDragging, unitBox }) {
  ...
  return (
    <>
        ...
        <TransformControls 
          ...
          onDraggingChanged={(e) => {
            ...
            if (!isDragging && groupRef.current) {
              ...
              onUpdate(obj.id, { position: { x: finalPos.x, y: finalPos.y, z: finalPos.z }, rotation: { y: finalRot.y } });
            }
            isGlobalDraggingRef.current = isDragging;
          ...
          onChange={(e) => {
            if (e.target.object) {
              ...
              let radiusX = (isRotated ? l : w) * 0.5, radiusZ = (isRotated ? w : l) * 0.5;
              if (isLine) { radiusX = isRotated ? 0.5 : l * 0.5; radiusZ = isRotated ? l * 0.5 : 0.5; }

              if (unitBox) {
                const WALL_MARGIN = 0.05; 
                target.position.x = THREE.MathUtils.clamp(target.position.x, unitBox.min.x + radiusX + WALL_MARGIN, unitBox.max.x - radiusX - WALL_MARGIN);
                target.position.z = THREE.MathUtils.clamp(target.position.z, unitBox.min.z + radiusZ + WALL_MARGIN, unitBox.max.z - radiusZ - WALL_MARGIN);
              }
      ...
      <group ref={groupRef} position={[position.x, position.y, position.z]} rotation={[0, rotation.y, 0]} onClick={(e) => { e.stopPropagation(); onSelect(obj.id); }}>
        {isLine ? (
          <>
            <Line points={[[-l/2, 0, 0], [l/2, 0, 0]]} color={color} lineWidth={12} />
            <mesh visible={false}><cylinderGeometry args={[0.5, 0.5, l, 8]} rotation={[0, 0, Math.PI / 2]} /><meshBasicMaterial transparent opacity={0} /></mesh>
          </>
        ) : <mesh castShadow receiveShadow><boxGeometry args={[w, h, l]} /><meshStandardMaterial color={color} emissive={isSelected ? color : "#000000"} emissiveIntensity={0.4} /></mesh>}
        {(type === 'machine' || isSelected) && (
          <Html distanceFactor={15} position={[0, isLine ? 1 : h/2 + 0.5, 0]} center>
            <div className={`text-white text-[10px] px-2 py-1 rounded-full whitespace-nowrap shadow-md pointer-events-none ${isSelected ? 'bg-blue-600 font-bold' : 'bg-slate-800/80'}`}>{obj.name}</div>
          </Html>
        )}
      </group>
    </>
...
export default function TvControlPanel() {
  const [tvSettings, setTvSettings] = useState({ 
    ...
    targetProject: "大成工業城", target3DUnit: "", view3DMode: "topdown", layout_objects: [], savedTemplates: [] 
  });
    ...
    const fetchData = async () => {
      try {
        ...
        if (tvSnap.exists()) {
          ...
          setTvSettings({ mode: 'image', layout_objects: [], savedTemplates: [], targetProject: "大成工業城", ...data, view3DMode: data.view3DMode || 'topdown' });
        }
      } catch (error) { console.error(error); } finally { setIsDataLoaded(true); setLoading(false); }
    };
    fetchData();
  }, []);

  const isFirstSyncSkipped = useRef(false);
  const modifySettings = (updater) => { setTvSettings(prev => typeof updater === 'function' ? updater(prev) : { ...prev, ...updater }); };

  useEffect(() => {
    if (!isDataLoaded || loading) return; 
    if (!isFirstSyncSkipped.current) { isFirstSyncSkipped.current = true; return; }
  ...
  const handlePlaceElement = (clickPoint) => {
    ...
    if (isMachine) {
      ...
      modifySettings(prev => ({ ...prev, layout_objects: [...(prev.layout_objects || []), newObj] }));
      ...
      if (!placementMode.startPoint) {
        ...
        const dx = safeX - start.x, dz = safeZ - start.z;
        const distance = Math.sqrt(dx*dx + dz*dz);
        const angleY = Math.atan2(-dz, dx); 
        let targetY = floorY + 0.3; if (placementMode.type === 'water') targetY = floorY + 0.05;

        const newObj = {
          ...placementMode, dimensions: { ...placementMode.dimensions, l: distance }, 
          position: { x: (start.x + safeX) / 2, y: targetY, z: (start.z + safeZ) / 2 }, rotation: { y: angleY }
        };
        modifySettings(prev => ({ ...prev, layout_objects: [...(prev.layout_objects || []), newObj] }));
        setPlacementMode(null); document.body.style.cursor = 'auto'; setSelectedObjId(newObj.id); 
      }
  ...
  const updateObject = (id, newTransform) => { modifySettings(prev => ({ ...prev, layout_objects: prev.layout_objects.map(obj => obj.id === id ? { ...obj, ...newTransform } : obj) })); };
  const deleteObject = (id) => { modifySettings(prev => ({ ...prev, layout_objects: prev.layout_objects.filter(obj => obj.id !== id) })); setSelectedObjId(null); };
  const updateObjectDimensions = (id, newDims) => { modifySettings(prev => ({ ...prev, layout_objects: prev.layout_objects.map(obj => obj.id === id ? { ...obj, dimensions: { ...obj.dimensions, ...newDims } } : obj) })); };
  const setExactRotation = (id, angleDegree) => { updateObject(id, { rotation: { y: (angleDegree * Math.PI) / 180 } }); };
  ...
  const handleSaveAsTemplate = () => {
    ...
    modifySettings(prev => ({ ...prev, savedTemplates: [...(prev.savedTemplates || []), { id: Date.now().toString(), name: templateName, objects: templateObjects }] }));
  };
  ...
  const handleApplyTemplate = () => {
    ...
    modifySettings(prev => ({ ...prev, layout_objects: [...(prev.layout_objects || []), ...newObjects] }));
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto font-sans h-[calc(100vh-80px)] overflow-y-auto">
      ...
      <div className="flex gap-4 mb-6 bg-slate-100 p-2 rounded-2xl">
        <button onClick={() => modifySettings({ mode: 'image' })} className={`flex-1 py-3 rounded-xl font-bold transition flex items-center justify-center gap-2 ${tvSettings.mode === 'image' ? 'bg-white shadow text-orange-600' : 'text-slate-500'}`}><Tv /> 照片輪播模式</button>
        <button onClick={() => modifySettings({ mode: '3d' })} className={`flex-1 py-3 rounded-xl font-bold transition flex items-center justify-center gap-2 ${tvSettings.mode === '3d' ? 'bg-white shadow text-blue-600' : 'text-slate-500'}`}><Box /> 3D 互動規劃模式</button>
      </div>
        ...
        <div className="flex flex-col xl:flex-row gap-6 h-[800px]">
          <div className="w-full xl:w-80 flex flex-col gap-4 overflow-y-auto pr-2 scrollbar-hide">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 shrink-0">
              <label className="block text-sm font-bold text-slate-500 mb-2">1. 案場與展示視角</label>
              
              {/* 🌟 新增案場選擇 */}
              <select value={tvSettings.targetProject} onChange={(e) => modifySettings({ targetProject: e.target.value })} className="w-full p-3 border-2 border-slate-100 rounded-xl focus:border-blue-500 outline-none font-bold mb-3 bg-slate-50">
                <option value="大成工業城">🏢 大成工業城</option>
                <option value="其他案場預留">🏢 其他案場 (擴充預留)</option>
              </select>

              <input type="text" value={tvSettings.target3DUnit} onChange={(e) => modifySettings({ target3DUnit: e.target.value.toUpperCase() })} placeholder="輸入戶號 (例如: A1-01)" className="w-full p-3 border-2 border-slate-100 rounded-xl focus:border-blue-500 outline-none uppercase font-bold mb-3" />
              
              {/* 🌟 補回全區俯視 */}
              <select value={tvSettings.view3DMode} onChange={(e) => modifySettings({ view3DMode: e.target.value })} className="w-full p-3 border-2 border-slate-100 rounded-xl focus:border-blue-500 outline-none font-bold">
                <option value="global">🌍 全區俯視 (宏觀視角)</option>
                <option value="topdown">📐 佈局俯視 (推薦編輯)</option>
            ...
            {selectedObject && (
              <div className="bg-blue-50 border border-blue-200 p-5 rounded-2xl shrink-0 animate-fade-in">
                ...
                <div className="flex gap-2 bg-white p-1 rounded-lg border border-blue-100 shadow-sm mb-4">
                  <button onClick={() => setTransformMode('translate')} className={`flex-1 py-1.5 text-xs font-bold flex items-center justify-center gap-1 rounded ${transformMode === 'translate' ? 'bg-blue-500 text-white' : 'text-slate-500'}`}><Move size={14}/> 移動</button>
                  <button onClick={() => setTransformMode('rotate')} className={`flex-1 py-1.5 text-xs font-bold flex items-center justify-center gap-1 rounded ${transformMode === 'rotate' ? 'bg-blue-500 text-white' : 'text-slate-500'}`}><RotateCw size={14}/> 旋轉</button>
                </div>
